...
const StockInList = () => {
    ...
    const handleGetStockInReport = async () => {
        try {
           const res = await axios.get('http://localhost:3000/stockIn/list', { withCredentials: true });
    ...
    const handleDeleteStockOut = async (_id) => {
        try {
            ...
            if (confrim ){
                await axios.delete(`http://localhost:3000/stockIn/delete/${_id}`, { withCredentials: true });
        ...
        <div className="bg-blue-50 min-h-screen">
            <div className="max-w-7xl mx-auto w-ful">
                <div className="flex justify-between mb-5 ">
                    <h1 className="text-xl font-bold text-blue-500">Stock In List</h1>
                    <Link to={'/stockIn/add'} className="bg-amber-500  py-3 px-6 rounded-lg text-white font-bold">Add new</Link>
                </div>
